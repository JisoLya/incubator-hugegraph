...
import org.apache.hugegraph.store.util.StoreTestUtil;
...
import static org.apache.hugegraph.store.client.HgStoreClientTest.EMPTY_BYTES;
...
import static org.apache.hugegraph.store.util.StoreTestUtil.*;


public class SessionManagerTest extends BaseTest {

    public static final String GRAPH_NAME_X = "default/hugegraph/x";
    public static final String TABLE_NAME_1 = defaultTable;
    public static HgStoreClient storeClient;
    private static PDClient pdClient;

    @BeforeClass
    public static void init() {
        pdClient = PDClient.create(getPdConfig());
    ...
    public void putGet() {
        ...
        batchPut(session, TABLE_NAME, "BATCH-PUT-TEST", batchSize);
    ...
    public void putGet2() {
        ...
        byte[] value = toBytes("FOR-PUT-VALUE");
        long stime = System.currentTimeMillis();
        batchPut(session, "testTable", "BATCH-PUT-TEST", batchSize);
        System.out.println("Time is " + (System.currentTimeMillis() - stime));
    }
    ...
    public void testBatchPutMultiGraph() throws IOException {
        ...
        batchPut(session1, TABLE_NAME_1, keyPrefix1, batchSize);
    ...
    public void paging() {
        ...
        batchPut(session, defaultTable, keyName, keyAmt);
        int count = 0;
        int limit = 0;
        int max = 99999;
        HgKvIterator<HgKvEntry> iterator = null;
        ...
        iterator = session.scanIterator(defaultTable, 100);
    ...
    public void truncate() {
        ...
        HgStoreSession session = getStoreSession(graph);
        batchPut(session, tableName, keyName, batchSize);
        Assert.assertEquals(batchSize, amountOf(session.scanIterator(tableName)));
        session.truncate();
    ...
    public void scanTable() {
        ...
        println(session.scanIterator("g+v", 10));
    }
    ...
    public void benchmarkScanBatch() {
        ...
        Assert.assertEquals(batchSize, StoreTestUtil.amountIn(iterators));
    }
    ...
    public void benchmarkScanBatch2() throws IOException {
        ...
        HgStoreSession session = getStoreSession();
        String keyName = "SCAN-BATCH";
        Map<HgOwnerKey, byte[]> data = batchPut(session, tableName, keyName, batchSize);
        println("-- Starting scan --");
        MetricX metrics = MetricX.ofStart();
        // HgStoreTestUtil.println(session.scanIterator(tableName));
